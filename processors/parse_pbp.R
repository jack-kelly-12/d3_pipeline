...
determine_r1_name = function(bat_text, bat_name, r1_text, r1_name, inn_end, game_end, sub_in, sub_out){
  m = length(bat_text)
  r1_name = character(m)
  for (i in 2:m){
    if (is.na(inn_end[i-1]) || is.na(game_end[i-1])) {
      r1_name[i] = NA_character_
      next
    }
...
determine_r2_name = function(bat_text, bat_name, r1_text, r1_name, r2_text, r2_name, inn_end, game_end, sub_in, sub_out){
  m = length(bat_text)
  r2_name = character(m)
  
  for (i in 2:m){
    if (is.na(inn_end[i-1]) || is.na(game_end[i-1])) {
      r2_name[i] = NA_character_
      next
    }
    ...
    if (inn_end[i-1] == 0 & game_end[i-1] == 0){
      r2_name[i] = case_when(
        ...
        ((str_detect(bat_text[i-1], '(doubled|advanced to second|stole second)') == TRUE) & 
           (str_detect(bat_text[i-1], '(advanced to third|scored|out|stole third)') == FALSE)) ~ stripwhite(gsub('((doubled|advanced to second|stole second).*$)', '', bat_text[i-1])),
        ((str_detect(r1_text[i-1], '(advanced to second|stole second)') == TRUE) & 
           (str_detect(r1_text[i-1], '(advanced to third|scored|out|stole third)') == FALSE)) ~ stripwhite(gsub('((advanced to second|stole second).*$)', '', r1_text[i-1])),
        r2_text[i-1] == '' & 
          stripwhite(gsub('((stole third|advanced to third|scored|out).*$)', '', r1_text[i-1])) != stripwhite(r2_name[i-1]) & 
          (str_detect(bat_text[i-1], '(advanced to third|stole third|scored|picked off|caught stealing)') == FALSE) ~ r2_name[i-1],
        r2_text[i-1] == '' & 
          stripwhite(gsub('((out on the play).*$)', '', r1_text[i-1])) != stripwhite(r2_name[i-1]) & 
          (str_detect(bat_text[i-1], '(double play)') == TRUE) ~ r2_name[i-1],
        r1_text[i-1] == '' & 
          (str_detect(bat_text[i-1], '(stole third|advanced to third|scored|picked off|homered|caught stealing)') == FALSE) ~ r2_name[i-1],
        TRUE ~ NA_character_
      )
    } else {
      r2_name[i] = NA_character_
  ...
  return(stripwhite(gsub('((singled|reached).*$)', '', r2_name)))
}

determine_r3_name = function(bat_text, bat_name, r1_text, r1_name, r2_text, r2_name, r3_text, r3_name, inn_end, game_end, sub_in, sub_out){
  m = length(bat_text)
  r3_name = character(m)
  
  for (i in 2:m){
    if (is.na(inn_end[i-1]) || is.na(game_end[i-1])) {
      r3_name[i] = NA_character_
      next
    }
    ...
    if (inn_end[i-1] == 0 & game_end[i-1] == 0){
      r3_name[i] = case_when(
        ...
        ((str_detect(bat_text[i-1], '(tripled|advanced to third|stole third)') == TRUE) & 
           (str_detect(bat_text[i-1], '(scored|out)') == FALSE)) ~ stripwhite(gsub('((tripled|advanced to third|stole third).*$)', '', bat_text[i-1])),
        ((str_detect(r1_text[i-1], '(advanced to third|stole third)') == TRUE) & 
           (str_detect(r1_text[i-1], '(scored|out)') == FALSE)) ~ stripwhite(gsub('((advanced to third|stole third).*$)', '', r1_text[i-1])),
        ((str_detect(r2_text[i-1], '(advanced to third|stole third)') == TRUE) & 
           (str_detect(r2_text[i-1], '(scored|out)') == FALSE)) ~ stripwhite(gsub('((advanced to third|stole third).*$)', '', r2_text[i-1])),
        r1_text[i-1] == '' & (str_detect(bat_text[i-1], '(scored|stole home|homered)') == FALSE) ~ r3_name[i-1],
        r2_text[i-1] == '' & 
          stripwhite(gsub('((scored|stole home|out).*$)', '', r1_text[i-1])) != stripwhite(r3_name[i-1]) & 
          (str_detect(bat_text[i-1], '(scored|stole home)') == FALSE) ~ r3_name[i-1],
        r3_text[i-1] == '' & 
          (str_detect(r2_text[i-1], '(scored|stole home|out)') == FALSE) & 
          (str_detect(r1_text[i-1], '(scored|stole home|out)') == FALSE) & 
          (str_detect(bat_text[i-1], '(scored|stole home)') == FALSE) ~ r3_name[i-1],
        TRUE ~ NA_character_
      )
    } else {
  ...
  return(stripwhite(gsub('((singled|doubled|reached|advanced|stole|failed|Failed|picked off).*$)', '', r3_name)))
}
...
outs_before = function(outs_on_play, new_game, new_inn) {
  ...
  inn_outs = integer(m)
  for (i in 2:m) {
    if (!is.na(new_game[i]) && !is.na(new_inn[i]) && new_game[i] == 0 && new_inn[i] == 0) {
...
score_before <- function(new_game, runs_on_play, top_inning, home_team = 1) {
  ...
  for (i in 2:m) {
    # First check if any values are NA
    if (is.na(new_game[i]) || is.na(top_inning[i-1]) || is.na(runs_on_play[i-1])) {
...
runs_play = function(home_score, away_score, home_score_before, away_score_before, top_inn){
  ...
  runs_play = integer(n)
  for (i in 2:n){
    case_when(top_inn[i] == 0 ~ home_score[i] - home_score_before[i])
    if (top_inn[i] == 0){
      runs_play[i] = home_score[i] - home_score_before[i]
    } else{
      runs_play[i] = away_score[i] - away_score_before[i]
    }
  }
  return(runs_play)
}
...
ncaa_parse_parallel <- function(pbp_data_frame, num_cores = detectCores() - 1) {
  # Split the data frame into chunks
  chunks <- split(pbp_data_frame, cut(seq(nrow(pbp_data_frame)), num_cores, labels = FALSE))
  
  # Create a cluster
  cl <- makeCluster(num_cores)
  
  # Export necessary functions and libraries to the cluster
  clusterExport(cl, c("stripwhite", "strip_punc", "inn_end", "game_end", "runs_on_play",
                      "determine_r1_name", "determine_r2_name", "determine_r3_name",
                      "new_game", "new_inn", "outs_before", "score_before",
                      "runs_play", "runs_this_inn", "runs_rest_of_inn",
                      "bat_order_id", "bat_order_fill"))
  ...
  results <- parLapply(cl, chunks, function(chunk) {
    chunk %>%
      mutate(
        ...
        # Runner names
        r1_name = determine_r1_name(bat_text, bat_name, r1_text, r1_name, inn_end, game_end, sub_in, sub_out),
        r2_name = determine_r2_name(bat_text, bat_name, r1_text, r1_name, r2_text, r2_name, inn_end, game_end, sub_in, sub_out),
        r3_name = determine_r3_name(bat_text, bat_name, r1_text, r1_name, r2_text, r2_name, r3_text, r3_name, inn_end, game_end, sub_in, sub_out),
        # Clean runner names
